...
try:
    from teacher_dashboard_qt import TeacherDashboard
except ImportError:
    from client.teacher_dashboard_qt import TeacherDashboard
    ...
    def handle_login(self):
        ...
        if resp.get("status") == "success":
            ...
            if user['role'] == 'teacher':
                self.open_teacher_dashboard(user)
            else:
                QMessageBox.information(self, "提示", f"学生端暂未重构为PyQt版本。\n用户: {user['name']}")
        else:
